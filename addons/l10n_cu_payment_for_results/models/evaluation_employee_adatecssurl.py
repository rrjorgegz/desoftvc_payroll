...
class EvaluationEmployeeADATECSsurl(models.Model):
    ...
    entry_work_id = fields.Many2one(
        "entry.work.adatecssurl", string="Entradas de Trabajo ADATECSsurl"
    )
    not_employees_ids = fields.Many2many(
        "hr.employee", compute="_compute_not_employees_ids"
    )
    employee_id = fields.Many2one(
        "hr.employee", string="Empleado", domain="[('id','not in',not_employees_ids)]"
    )
    contract_id = fields.Many2one(
        "hr.contract", string="Contrato", domain="[('state','=','open')]", index=True
    )
    ...
    def create(self, vals_list):
        vals_list["name"] = self.env["ir.sequence"].next_by_code(
            "evaluation.employee.adatecssurl"
        )
    ...
    def onchange_scale_group_id(self):
        ...
        scale = self.scale_group_id.scale_group_line_ids.filtered(
            lambda li: self.evaluation >= li.eval_start
            and self.evaluation <= li.eval_end
        )
        wage = self.scale_group_id.salary
        if len(scale) == 1:
            wage = scale.currency._convert(
                from_amount=scale.salary, to_currency=self.currency_id
            )
