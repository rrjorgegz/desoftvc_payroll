...
class HrEmployee(models.Model):
    ...
    expediente = fields.Integer(
        string="Número del expediente", default=00000, required=True
    )
    ...
    type_retired_id = fields.Many2one(
        "classification.retired.workers", string="Tipo de retiro"
    )

    register_date = fields.Date("Fecha de alta")
    discharge_date = fields.Date("Fecha de baja")
    discharge_reason = fields.Many2one(
        comodel_name="causa.baja", string="Causa de la baja"
    )
    ...
    document_require = fields.Many2many(
        "necesary.docs", string="Documentación necesaria"
    )
    ...
    def _concatFullName(self):
        ...
        self.name = (
            ffn.strip() + " " + fln.strip() + " " + sfn.strip() + " " + sln.strip()
        )
        ...
        if self.working_ok:
            if not self.discharge_reason:
                raise exceptions.ValidationError(
                    _("Debe llenar la causa de la baja antes de dar baja al trabajador")
                )

            if (
                not self.discharge_date
                or datetime.strptime(
                    str(self.discharge_date), tools.DEFAULT_SERVER_DATE_FORMAT
                )
                > datetime.now()
            ):
